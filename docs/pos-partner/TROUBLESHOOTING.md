...
## Table QR and lifecycle

If a QR shows no active bill, confirm the POS has upserted an open bill for the mapped `external_table_id`. `table_active_bill_conflict` means another open bill owns the table; close/cancel or move it through normal POS operations. Do not edit bill rows manually. A customer visit that has ended must be replaced by a new QR scan. A rotated QR must be reprinted; a disabled table must be enabled by an operator.
...
| `payment_capacity_conflict` | Another completed or protected payment reduced available capacity; reconcile the bill             |
| `payment_outcome_ambiguous` | Do not create a new payment ID; retry exact bytes/key and escalate if reconciliation times out    |
