...
2. Apply `supabase/migrations/20260723000100_payment_sessions.sql` to the sandbox project using the team's normal `supabase db push --linked` or reviewed SQL migration process. Confirm the linked project is sandbox before approval.
...
4. Deploy the exact tested commit to the existing sandbox API project. Do not deploy production.
